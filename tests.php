...
Если они между собой равны, значит это палиндром. Для переворота строки 
воспользуйтесь функцией reverse, которая находится в файле src/Strings.php.
...
В отличие от предыдущей реализации, выполните эту более простым способом, 
через сравнение исходной строки с ее перевернутой версией. Если они между собой равны, 
значит это палиндром. Для переворота строки воспользуйтесь функцией reverse(), 
которая находится в файле src/Strings.php.
...
function isPalindrome($string)
{
    return $string == \Strings\reverse($string) ? true : false;
}
